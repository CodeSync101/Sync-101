...
public class RapportService {
    ...
    @Autowired
    private CommitReportRepository commitRepository;

    @Autowired
    private PullRequestReportRepository pullRequestRepository;
    ...
    @Autowired
    private BranchReportRepository branchRepository;
    ...
    public ByteArrayInputStream generatePdfRapport(LocalDate startDate, LocalDate endDate, List<String> selectedStudents, List<String> selectedSections) {
        ...
        List<PullRequestReport> pullRequests = pullRequestRepository.findAll();
        List<IssueReport> issues = issueRepository.findAll();
    ...
    private List<PullRequestReport> filterPullRequests(List<PullRequestReport> prs, LocalDate startDate, LocalDate endDate, List<String> selectedStudents) {
    ...
    private List<IssueReport> filterIssues(List<IssueReport> issues, LocalDate startDate, LocalDate endDate, List<String> selectedStudents) {
    ...
    private void addPullRequestsSection(Document document, List<PullRequestReport> pullRequests, Font headFont, Font bodyFont) throws DocumentException, IOException {
        ...
        for (PullRequestReport pr : pullRequests) {
    ...
    private void addIssuesSection(Document document, List<IssueReport> issues, Font headFont, Font bodyFont) throws DocumentException, IOException {
    ...
    private void addDecisionSection(Document document,
                                    ...
                                    List<Object[]> stats,
                                    List<PullRequestReport> pullRequests,
    ...
    private JFreeChart createPullRequestsByStateChart(List<PullRequestReport> pullRequests) {
        DefaultPieDataset dataset = new DefaultPieDataset();
        Map<String, Integer> stateCounts = pullRequests.stream()
                .collect(Collectors.groupingBy(PullRequestReport::getEtat, Collectors.summingInt(e -> 1)));
    ...
    private JFreeChart createIssuesByStateChart(List<IssueReport> issues) {
        DefaultPieDataset dataset = new DefaultPieDataset();
        Map<String, Integer> stateCounts = issues.stream()
                .collect(Collectors.groupingBy(IssueReport::getEtat, Collectors.summingInt(e -> 1)));
        stateCounts.forEach((state, count) -> {
            if (count > 0) dataset.setValue(state, count);
        });
        JFreeChart chart = ChartFactory.createPieChart(
